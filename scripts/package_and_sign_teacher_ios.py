...
def main():
    ...
    assets_xcassets = os.path.join(root_dir, "speaxa_teacher", "ios", "Runner", "Assets.xcassets")
    ...
    print("--- 2. Compiling Assets.car with New AppIcon (from Android) ---")
    compiled_assets_dir = "/tmp/compiled_teacher_assets"
    if os.path.exists(compiled_assets_dir):
        shutil.rmtree(compiled_assets_dir)
    os.makedirs(compiled_assets_dir, exist_ok=True)
    ...
    run_cmd([
        ...
        '--app-icon', 'AppIcon',
        '--output-partial-info-plist', '/tmp/app_icon_partial.plist',
        assets_xcassets
    ])

    for item in os.listdir(compiled_assets_dir):
        src_item = os.path.join(compiled_assets_dir, item)
        dst_item = os.path.join(app_path, item)
        if os.path.isfile(src_item):
            shutil.copy2(src_item, dst_item)
            print(f"Copied {item} to Runner.app")

    print("--- 3. Updating Info.plist and Framework Plists (Xcode 16.2 GM) ---")
    ...
    print("--- 4. Updating Mach-O SDK Headers with vtool (iOS 18.2 GM) ---")
    ...
    print("--- 5. Creating Production Entitlements ---")
    entitlements = {
    ...
    print("--- 7. Signing Main Application Bundle ---")
    run_cmd([
        'codesign',
        '-f',
        '-s', cert_name,
